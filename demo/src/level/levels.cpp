...
namespace demo::level {
    ...
    constexpr u8 kPrevParityFix = (video::viewport_tx() >> 1) & 1u;   // viewport_mx() & 1
    ...
    u8 GetPrevWrite(const u8 step) {
        if (~step & 1) {
            if (step == 0) {
                attr_column++;
                const u8 mask = (attr_column + kPrevParityFix) & 1 ? 0xCC : 0x00;
                for (auto & j : AttributeBuffer)
                    j &= mask;
            }
            ...
            const u8 pal       = Metatiles_ATTR[MetatileBuffer[step >> 1]] & MetatilePaletteMask;
            const u8 is_bottom = tile_row >> 1 & 1;
            const u8 shift     = (attr_column + kPrevParityFix) & 1
                                    ? (is_bottom ? 4 : 0)
                                    : is_bottom ? 6 : 2;
            AttributeBuffer[attr_idx] |= pal << shift;
        }
    ...
    void BuildPrevColumn(u8* buf, const u16 worldCol) {
        attr_column++;
        const u8 ap = (attr_column + kPrevParityFix) & 1;   // odd-width parity fix
