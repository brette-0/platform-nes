...
namespace ui::text {
    namespace {
        ...
        u8 WrapLineEnd(const u8* buff, u8 sBuff, u8 start, u8 boxX, u8 splitter);
    }

    u8 Draw(const u8* buff, const u8 sBuff, const vec2<u16> pos, const vec2<u8> box, const u8 splitter, const Alignment align) {
        u8 cursor = 0;
        u8 last   = 0;
        u8 row    = 0;
    ...
    ContinuousDrawer::ContinuousDrawer(
    ...
    u8 ContinuousDrawer::AdvanceToNextLine(const u8 offset) const {
        if (offset >= sBuff) return sBuff;
        return WrapLineEnd(buff, sBuff, offset, box.x, splitter);
    }

    u8 ContinuousDrawer::AdvanceToPreviousLine(const u8 offset) const {
        if (!offset) return 0;
    ...
    bool ContinuousDrawer::Previous() {
    ...
    namespace {
        u8 WrapLineEnd(const u8* buff, const u8 sBuff, const u8 start, const u8 boxX, const u8 splitter) {
            u8   cursor     = start;
            u8   lastWhite  = start;
            bool foundWhite = false;
