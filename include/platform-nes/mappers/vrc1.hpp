...
/// Named fixed_bank_tag, not fixed_tag: BANKED()'s tag argument gets
/// macro-expanded before tag##_tag pastes it (so a #define'd tag can be
/// forwarded through config headers, see BANKED_IMPL below) -- but that
...
/// and would expand before pasting, producing garbage. BANKED() call sites
/// for this domain must use the tag `fixed_bank`, not `fixed`.
...
/// One specialization per BANKED()/BANKED_EXTERN() call site, generated by
/// those macros -- never written by hand. Primary template deliberately
/// undefined, same reasoning as bank_layout<Tag>.
...
namespace vrc1_detail {
    ...
    template <typename TReturn, typename TFunc>
    [[gnu::noinline]] fixed TReturn CallInWindows2(u8 windowIndexBase, u8 bankBase, TFunc fn) {
 ...
 *        bank_of<>, so Call<Fn>/CallBlock<Fn> can resolve its bank later.
 *
 ...
 * characters. The BANKED -> BANKED_IMPL indirection is load-bearing: it
 * forces section_name/tag to be macro-expanded to their final literal text
 * before # or ## ever sees them, so a #define'd section_name/tag (see
 * BANKED_EXTERN below) stringizes/pastes correctly instead of pasting the
 * macro's own name.
 */
#define BANKED(section_name, tag, ret, name, ...) \
    BANKED_IMPL(section_name, tag, ret, name, __VA_ARGS__)
#define BANKED_IMPL(section_name, tag, ret, name, ...)                     \
    CREATE_SEGMENT_KEYWORD(section_name) ret name(__VA_ARGS__);            \
    template <> struct bank_of<static_cast<ret (*)(__VA_ARGS__)>(&name)> { \
        using layout = bank_layout<tag##_tag>;                             \
    }

/**
 * @brief Registers a function with bank_of<> without declaring/placing it.
 *
 ...
 */
#define BANKED_EXTERN(section_name, tag, ret, name, ...) \
    BANKED_EXTERN_IMPL(section_name, tag, ret, name, __VA_ARGS__)
#define BANKED_EXTERN_IMPL(section_name, tag, ret, name, ...)              \
...
/**
 * @brief Calls a specific, compile-time-known BANKED() function, resolving
 *        its bank from bank_of<Fn> automatically -- no manual window/bank
 *        argument, unlike ::Long.
 *
 * @tparam Fn Address of a BANKED()/BANKED_EXTERN()-tagged function --
 *            supplied explicitly, e.g. `Call<LoadLevelChunk>(chunkId)`.
 */
...
fixed auto Call(Args &&...args) -> typename function_traits<decltype(Fn)>::return_type {
    ...
    if constexpr (L::always_mapped) {
        ...
        return vrc1_detail::CallInSection<TReturn>(
            L::section(),
...
fixed auto CallBlock(Block &&block) -> decltype(block()) {
    ...
    if constexpr (L::always_mapped) {
        ...
        return vrc1_detail::CallInSection<decltype(block())>(
